...
import vasco.ParameterAnalysis;
 ...
 *
 */
public class SignAnalysis extends ForwardInterProceduralAnalysis<SootMethod, Unit, SignMap> {
	...
	private Sign signOf(Value value, SignMap dfv) {
	...
	private void assign(Local lhs, Value rhs, SignMap input, SignMap output) {
	...
	private void assign(Local lhs, Value rhs, SignMap input, SignMap output, SootMethod calledMethod) {
		// We only care about numeric locals
		if (lhs.getType() instanceof IntType && ParameterAnalysis.localMap.containsKey(calledMethod) && ParameterAnalysis.localMap.get(calledMethod).get(lhs)) {
			// First remove casts, if any.
			if (rhs instanceof CastExpr) {
				rhs = ((CastExpr) rhs).getOp();
			}	
	...
	@Override
	public SignMap normalFlowFunction(
			Context<SootMethod, Unit, SignMap> context, Unit unit,
			SignMap inValue) {
		// Initialize result to input
		SignMap outValue = copy(inValue);
	...
	@Override
	public SignMap callEntryFlowFunction(
			Context<SootMethod, Unit, SignMap> context, SootMethod calledMethod, Unit unit,
			SignMap inValue) {
		// Initialise result to empty map
		SignMap entryValue = topValue();
		// Map arguments to parameters
		InvokeExpr ie = ((Stmt) unit).getInvokeExpr();
		for (int i = 0; i < ie.getArgCount(); i++) {
			Value arg = ie.getArg(i);
			Local param = calledMethod.getActiveBody().getParameterLocal(i);
			assign(param, arg, inValue, entryValue, calledMethod);
		}
		...
		if (ie instanceof InstanceInvokeExpr) {
			...
			assign(thisLocal, instance, inValue, entryValue, calledMethod);
		}
	...
	@Override
	public SignMap callExitFlowFunction(Context<SootMethod, Unit, SignMap> context, SootMethod calledMethod, Unit unit, SignMap exitValue) {
		// Initialise result to an empty value
		SignMap afterCallValue = topValue();
	...
	@Override
	public SignMap callLocalFlowFunction(Context<SootMethod, Unit, SignMap> context, Unit unit, SignMap inValue) {
		// Initialise result to the input
		SignMap afterCallValue = copy(inValue);
	...
	@Override
	public SignMap boundaryValue(SootMethod method) {
		return topValue();
	}

	@Override
	public SignMap copy(SignMap src) {
		return new SignMap(src);
	}



	@Override
	public SignMap meet(SignMap op1, SignMap op2) {
		SignMap result;
		// First add everything in the first operand
		result = new SignMap(op1);
